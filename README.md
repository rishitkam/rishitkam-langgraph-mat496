...
Tweakings : I created a new graph of my own, which generates 5 coding questions based on a topic at once in parallel. and then chooses the hardest one among them to display as the final result, I did this using the map reduce features using a function called hardest_question which I added in the notebook, furthermore I made a new py file and added it to the langgraph.json file to access my own graph through the langgraph studio, I have attached the screenshots of the changes i did to langgraph.json and the py file , I then analysed this graph stepwise and in depth on the langgraph studio and also went through the traces using our og langsmith portal, I attached the screenshot of my analysis of both the langgraph studio and the langsmith portal along with markdown explanations with them to convey aptly what i understood and inferred.
